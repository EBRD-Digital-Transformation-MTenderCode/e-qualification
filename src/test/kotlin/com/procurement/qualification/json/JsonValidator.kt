...
fun <T : Any> testingBindingAndMapping(pathToJsonFile: String, target: Class<T>) {
    ...
    try {
        ...
        fail<T>("Error testing binding and mapping JSON file by path: '$pathToJsonFile' " +
                    "to an object of type '${target.canonicalName}'.\n${exception.message}")
    }
}
    ...
    fun equalsJsons(expectedJson: String, actualJson: String, additionalChecks: (JsonValues.() -> Unit)? = null) {
        ...
        if (expectedData.size != actualData.size) {
            ...
            val message = buildString {
                appendln("Error comparing JSONs. " +
                             "Different number of attributes (expected json: ${expectedData.size} attributes, " +
                             "actual json: ${actualData.size} attributes)")
