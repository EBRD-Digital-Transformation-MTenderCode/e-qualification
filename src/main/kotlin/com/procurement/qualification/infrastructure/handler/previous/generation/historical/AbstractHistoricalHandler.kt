...
abstract class AbstractHistoricalHandler<ACTION : CommandType, R : Any>(
    ...
    override fun handle(node: JsonNode): ApiResponse {
        ...
        val history = historyRepository.getHistory(cm.id, action.key)
            .doReturn { incident ->
                return apiResponseGenerator.generateResponseOnFailure(fail = incident, id = cm.id, version = cm.version, logger = logger)
            }
        ...
        if (history != null) {
            ...
            val data = jsonData.tryToObject(target)
                .doReturn { incident ->
                    return apiResponseGenerator.generateResponseOnFailure(
